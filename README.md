...
- **`statistical_tests/`**  
  Files for the statistical analysis of the experiment results in R/RStudio, including:
  - `all_results_cleaned.csv` – Cleaned dataset used for statistical evaluation
  - `Statistische tests.Rproj` – RStudio project for the statistical analysis workflow
...
- R and optionally RStudio (for the statistical analysis in `statistical_tests/`)
...
### 6. Statistical Analysis Setup (R)

The repository also contains a separate R-based analysis workspace in `statistical_tests/`.

- Open `statistical_tests/Statistische tests.Rproj` in RStudio to load the statistical analysis environment.
- Use `statistical_tests/all_results_cleaned.csv` as the cleaned input dataset for the statistical evaluation.
- If your local R environment does not already have the required packages installed, install them in R before running the analysis.
...
### 4) Run or inspect the statistical analysis in R

- Open `statistical_tests/Statistische tests.Rproj` in RStudio.
- Load or inspect `statistical_tests/all_results_cleaned.csv` for the final dataset used in the thesis analysis.
- Re-run the statistical evaluation from the RStudio project environment if you want to reproduce or adapt the analysis.
...
- [ ] (Optional) Installed R/RStudio for the statistical analysis in `statistical_tests/`
...
The `statistical_tests/` directory contains the files used for the thesis-level statistical evaluation in R.

- `all_results_cleaned.csv` contains the cleaned version of the aggregated experiment results.
- `Statistische tests.Rproj` provides the RStudio project context for reproducing or extending the statistical analysis.
